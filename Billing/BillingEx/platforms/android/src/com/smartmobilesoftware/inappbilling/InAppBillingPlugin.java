...
public class InAppBillingPlugin extends CordovaPlugin {
	...
	public boolean execute(String action, JSONArray data, final CallbackContext callbackContext) {
		...
		try {
			...
			}else if ("mapProducts".equals(action)) {
				JSONArray jsonList = new JSONArray(data.getString(0));
				final Bundle productMappings = new Bundle();			
				
				int len = jsonList.length();
				Log.d(TAG, "Num products to map Found: " + (len / 2));
				
	   			for (int i=0;i<len;i = i + 2){
	   				productMappings.putString(jsonList.get(i).toString(),jsonList.get(i+1).toString()); 
	    			Log.d(TAG, "Mapping Added: "+jsonList.get(i).toString() + " to " + jsonList.get(i+1).toString());
	   			}
	   			
	   			mapProducts(productMappings);
			} 
			else {
	...
	private void mapProducts(Bundle data) throws RemoteException{		
		mHelper.doProductMappings(data);
	}
